...
@section('content')
    <div
        style="background-color: #ffffff; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); border: 1px solid #e5e7eb; border-radius: 0.5rem; padding: 1.5rem 2rem; margin: 10px auto; max-width: 672px;">
        ...
        <form action="{{ route('barang.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="lab_id" id="lab" class="form-select">
                    @foreach ($lab as $lb)
                        <option value="{{ $lb->id }}">{{ $lb->nama_lab }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
